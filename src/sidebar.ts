...
export class HackerChatSidebarProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.html = getWebviewContent(indexPath.path).toString();
  ...
  public openPanel(data: any) {
    ...
    panel.webview.html = getWebviewContent("https://www.baidu.com").toString();
  }
...
function getWebviewContent(indexPath: string): string {
  ...
  function getHtmlForWebview(webview: vscode.Webview, extensionUri: vscode.Uri): string {
    const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'js', 'app.94afa799.js'));
    const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'css', 'app.2cf79ad6.css'));
